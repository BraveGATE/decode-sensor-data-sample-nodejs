...
class FloodSensorSetting {
  ...
  constructor(sensorSettingBase64) {
    ...
    if (this.aliveSetting.name !== 'Interval') {
      this.scheduleSetting = new ScheduleSetting(buf.subarray(15, 75), this.aliveSetting.name);
    } else {
      this.scheduleSetting = buf.readInt32LE(11);
    }
  ...
  toString() {
    return (
      ...
      `AliveSetting: ${this.aliveSetting.name}\n` +
...
class ScheduleSetting {
  ...
  constructor(bytes, scheduleType) {
    ...
    for (let i = 0; i < ScheduleSetting._size; i += intervalBytes) {
      ...
      if (scheduleType == 'Monthly') {
        ...
        const min = tmpMin % 60;
        this.monthlySchedule.push(new Date(2022, 1, day, hour, min));
      } else if (scheduleType == 'Daily') {
        if (0 == Buffer.from([0xff, 0xff]).compare(scheduleBytes)) continue;
        const tmpMin = scheduleBytes.readInt16LE(0);
        const hour = Math.floor(tmpMin / 60);
        const min = tmpMin % 60;
        this.dailySchedule.push(new Date(2022, 1, 1, hour, min));
      }
    }
  }
